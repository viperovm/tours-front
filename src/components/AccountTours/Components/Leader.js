...
import {
  setSecondaryNav, updateTour, getTourLeaders, updateGuestGuide, setGuestGuideImage, tourToServer,
...
const Leader = ({
  ...
  const handleSelectInput = (name, value) => {
    updateTour({
      ...tour, team_member: value, guest_guide: null,
    })
  }

  const handleButtonClearSelect = () => {
    setLoadingTeamMember(true)
    updateTour({
      ...tour, team_member: null,
    })
  }

  useEffect(() => {
    if (loadingTeamMember && !tour.team_member) {
      setLoadingTeamMember(false)
    }
  }, [tour, loadingTeamMember])

  const handleButtonClearGuide = () => {
    setLoadingGuide(true)
    updateTour({
      ...tour, guest_guide: null,
    })
  }

  useEffect(() => {
    if (loadingGuide && !tour.team_member) {
      ...
      <ToursEditLayout secondary_item='leader' secondary_name='Турлидер'>
        ...
        {loadingTeamMember ?
          ...
          (<>
              ...
              <Button
                color={`button-danger`}
                action={handleButtonClearSelect}
                text='Очистить'
              />
            </>)}

        {loadingGuide ? <CircularProgress/> :
        ...
        <div
          style={{
            display: 'flex', justifyContent: 'space-between', width: '66%',
          }}
        >
          <Link
...
const mapStateToProps = state => ({
  secondary_nav: state.tours.secondary_nav, tour: state.tours.current_tour, tour_leaders: state.tours.tour_leaders,
})
